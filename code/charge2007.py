...
############################################################################################
# create neighborhood complex start
...
def get_simplices_fast(N,cutoff,filtration,PI,LI):
    #####################################################################################
    ...
    ###################################################################################
    # create filtered neighbourhood complex, the component in protein
    ...
    ##################################################################################
    # create filtered neighbourhood complex, the component in ligand
...
def get_simplices_ligand(N,cutoff,filtration,PI,LI):
    #####################################################################################
    # read pocket coordinate
...
###########################################################################################
# create neighborhood complex ends


# get homology information
def get_one_homology(start,end):
    for i in range(start,end):
        name = all_data[i]
        for P in range(5):
            for L in range(10):
                complex1 = get_simplices_ligand(i,10,7,P,L)
                
                res = PH.get_persistence(complex1)
                if len(res)==0:
                    one_bar = np.array([])
                    filename1 = pre + 'charge_one_homology/' + name + '_ligand_' + Protein_Atom[P] + '_' + Ligand_Atom[L] + '_one_bar.csv'
                    np.savetxt(filename1,one_bar,delimiter=',')
                    continue
                diag = res['diagrams']
...
def eigenvalue_to_file(start,end,cutoff,filtration,grid):
    for i in range(start,end):
        ...
        for P in range(5):
            for L in range(10):
                simplices_p,simplices_l = get_simplices_fast(i,cutoff,filtration,P,L)
...
def train_feature_to_file(start,end,cutoff,grid):
    row = end - start
    N = 10
    number0 = 100
    number1 = 100
    ...
    filename = pre + 'pocket_feature/pre_charge_laplacian_train_50_10_10_' + str(grid) + '.csv'
    np.savetxt(filename,feature_matrix,delimiter=',')
    


def test_feature_to_file(start,end,cutoff,grid):
    row = end - start
    N = 10
    number0 = 100
    number1 = 100
    ...
    filename = pre + 'pocket_feature/pre_charge_laplacian_test_50_10_10_' + str(grid) + '.csv'
    np.savetxt(filename,feature_matrix,delimiter=',')
    
def get_homology_feature_of_train(start,end,filtration,unit):
    N = int(1/unit)
    N1 = 16
    # birth: max,min,ave,std,sum,count
    # death: max,min,ave,std,sum,count
    # persistence: (],[],[),() 
    column = 50 * N * N1
    feature_matrix = np.zeros((end-start,column))
    for i in range(start,end):
        name = train_data[i]
        count = 0
        for P in range(5):
            for L in range(10):
                filename2 = pre + 'charge_one_homology/' + name + '_ligand_' + Protein_Atom[P] +'_' + Ligand_Atom[L] + '_one_bar.csv'
                one_bar = np.loadtxt(filename2,delimiter=',')
                for n in range(N):
                    left = n * unit
                    right = n * unit + unit
                    ...
                    if len(t)==1:
                        for tt in range(N1):
                            feature_matrix[i][count] = 0
                            count = count + 1
                    else:
                        
                        for tt in range(t[0]):
                            if (one_bar[tt][0]>=left) and (one_bar[tt][0]<=right):
                                one_birth.append(one_bar[tt][0])
                            if (one_bar[tt][1]>=left) and (one_bar[tt][1]<=right):
                                one_death.append(one_bar[tt][1])
                            if (one_bar[tt][0]<left) and (one_bar[tt][1]<=right) and (one_bar[tt][1]>=left):
                                p1 = p1 + 1
                            if (one_bar[tt][0]>=left) and (one_bar[tt][0]<=right) and (one_bar[tt][1]>=left) and (one_bar[tt][1]<=right):
                                p2 = p2 + 1
                            if (one_bar[tt][0]>=left) and (one_bar[tt][0]<=right) and (one_bar[tt][1]>right):
                                p3 = p3 + 1
                            if (one_bar[tt][0]<left) and (one_bar[tt][1]>right):
                                p4 = p4 + 1
                        feature_matrix[i][count] = get_max(one_birth)
                        count = count + 1
                        feature_matrix[i][count] = get_min(one_birth)
                        count = count + 1
                        feature_matrix[i][count] = get_mean(one_birth)
                        count = count + 1
                        feature_matrix[i][count] = get_std(one_birth)
                        count = count + 1
                        feature_matrix[i][count] = get_sum(one_birth)
                        count = count + 1
                        feature_matrix[i][count] = len(one_birth)
                        count = count + 1
                        
                        feature_matrix[i][count] = get_max(one_death)
                        count = count + 1
                        feature_matrix[i][count] = get_min(one_death)
                        count = count + 1
                        feature_matrix[i][count] = get_mean(one_death)
                        count = count + 1
                        feature_matrix[i][count] = get_std(one_death)
                        count = count + 1
                        feature_matrix[i][count] = get_sum(one_death)
                        count = count + 1
                        feature_matrix[i][count] = len(one_death)
                        count = count + 1
            
                        feature_matrix[i][count] = p1
                        count = count + 1
                        feature_matrix[i][count] = p2
                        count = count + 1
                        feature_matrix[i][count] = p3
                        count = count + 1
                        feature_matrix[i][count] = p4
                        count = count + 1
                    
    filename3 = pre + 'pocket_feature/charge_ligand_one_homology_train.csv'
    np.savetxt(filename3,feature_matrix,delimiter=',')
        
    
def get_homology_feature_of_test(start,end,filtration,unit):
    N = int(1/unit)
    N1 = 16
    # birth: max,min,ave,std,sum,count
    # death: max,min,ave,std,sum,count
    ...
    filename3 = pre + 'pocket_feature/charge_ligand_one_homology_test.csv'
    np.savetxt(filename3,feature_matrix,delimiter=',')
  
    
def get_combined_feature(typ):
    filename1 = pre + 'pocket_feature/pre_charge_laplacian_' + typ + '_50_10_10_0.02.csv'
    filename2 = pre + 'pocket_feature/charge_ligand_one_homology_' + typ + '.csv'
    ...
    filename3 = pre + 'charge_laplacian_' + typ + '_50_10_10_0.02.csv'
...
def get_pearson_correlation(typ,pref):
    feature_matrix_of_train = np.loadtxt( pre + '/pocket_feature/' + pref +'charge_laplacian_train_50_10_10_0.02.csv',delimiter=',' )
    target_matrix_of_train = np.loadtxt( pre + '/pocket_feature/' + 'target_matrix_of_train.csv',delimiter=',' )
    feature_matrix_of_test = np.loadtxt( pre + '/pocket_feature/' + pref + 'charge_laplacian_test_50_10_10_0.02.csv',delimiter=',' )
...
def run_for_PDBbind_2007():
    ...
    # create neighborhood complex and compute the spectral and
    # homology information
    eigenvalue_to_file(0,1300,10,10,0.01)
    get_one_homology(0,1300)
    
    # feature generation
    train_feature_to_file(0,1105,10,0.02)
    test_feature_to_file(0,195,10,0.02)
    get_homology_feature_of_train(0,1105,7,0.05)
    get_homology_feature_of_test(0,195,7,0.05)
    get_combined_feature('train')
    get_combined_feature('test')
