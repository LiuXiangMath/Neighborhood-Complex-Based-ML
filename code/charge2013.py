...
def get_one_homology(start,end):
    for i in range(start,end):
        ...
        for P in range(5):
            for L in range(10):
                complex1 = get_simplices_ligand(i,10,10,P,L)
    ...
    for i in range(start,end):
        ...
        for P in range(5):
            for L in range(10):
                ...
                filename2 = pre + 'ligand_charge_barcode/' + name + '_ligand_' + Protein_Atom[P] +'_' + Ligand_Atom[L] + '_one_bar.csv'
                one_bar = np.loadtxt(filename2,delimiter=',')
               
                for n in range(N):    
                    feature_matrix[i][count] = get_number( one_bar , unit * n , unit * (n+1) )
                    count = count + 1
...
def run_for_PDBbind_2013():
    ...
    get_homology_feature_of_train(0,2764,10,0.02)
    get_homology_feature_of_test(0,195,10,0.02)
