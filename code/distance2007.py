...
def get_simplices_fast(N,cutoff,filtration,PI,LI):
    ####################################################################################
    '''
    this function create the filtered neighborhood complex of the bipartite graph
    ...
    for i in range(start,end):
        ...
        for P in range(4):
            for L in range(9):
                ...
                if len(res)==0:
                    one_bar = np.array([])
                    filename1 = pre + 'one_homology/' + name + '_ligand_' + Protein_Atom[P] + '_' + Ligand_Atom[L] + '_one_bar.csv'
...
def train_feature_to_file(start,end,cutoff,filtration,grid):
    ...
    filename = pre + 'pocket_feature/pre_laplacian_train_10_10_0.1.csv'
...
def test_feature_to_file(start,end,cutoff,filtration,grid):
    ...
    filename = pre + 'pocket_feature/pre_laplacian_test_10_10_0.1.csv'
    np.savetxt(filename,feature_matrix,delimiter=',')


def get_homology_feature_of_train(start,end,filtration,unit):
    N = int((filtration-2)/unit)
    N1 = 16
    ...
    for i in range(start,end):
        ...
        for P in range(4):
            for L in range(9):
                filename2 = pre + 'one_homology/' + name + '_ligand_' + Protein_Atom[P] +'_' + Ligand_Atom[L] + '_one_bar.csv'
    ...
    filename3 = pre + 'pocket_feature/ligand_one_homology_train.csv'
...
def get_homology_feature_of_test(start,end,filtration,unit):
    ...
    filename3 = pre + 'pocket_feature/ligand_one_homology_test.csv'
    np.savetxt(filename3,feature_matrix,delimiter=',')


def get_combined_feature(typ):
    filename1 = pre + 'pocket_feature/pre_laplacian_' + typ + '_10_10_0.1.csv'
    filename2 = pre + 'pocket_feature/ligand_one_homology_' + typ + '.csv'
    ...
    filename3 = pre + 'laplacian_' + typ + '_10_10_0.1.csv'
...
def get_pearson_correlation(typ,pref):
    feature_matrix_of_train = np.loadtxt( pre + 'pocket_feature/' + pref +'laplacian_train_10_10_0.1.csv',delimiter=',' )
    target_matrix_of_train = np.loadtxt( pre + 'pocket_feature/' + 'target_matrix_of_train.csv',delimiter=',' )
    feature_matrix_of_test = np.loadtxt( pre + 'pocket_feature/' + pref + 'laplacian_test_10_10_0.1.csv',delimiter=',' )
...
def run_for_PDBbind_2007():
    ...
    # create neighborhood complex and compute the spectral and homology
    # information
    ...
    get_homology_feature_of_train(0,1105,7,0.5)
    get_homology_feature_of_test(0,195,7,0.5)
